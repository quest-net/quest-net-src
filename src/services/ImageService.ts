...
export class ImageService {
  ...
  private sendImageUpload!: (data: ArrayBuffer, targetPeers: string | null, metadata: { name: string; width: number; height: number; mimeType: string; fileSize: number; uploadId: string }) => void;
  private sendImageCreated!: (data: { uploadId: string; imageId: string; name: string; fileSize: number; mimeType: string; width: number; height: number }, peerId: string) => void;

  constructor(room: Room, isDM: boolean, actionExecute: (actionKey: string, params: any) => void = () => {}) {
    this.room = room;
    this.isDM = isDM;
  ...
  private setupChannels() {
    ...
    if (this.isDM) {
      ...
      getUpload((data, peerId, metadata) => {
        ...
        console.log(`[ImageService] DM received upload from ${peerId}: ${meta.name}`);
  ...
  async uploadImage(file: File, name?: string): Promise<Image> {
    ...
    const promise = this.processAndUpload(file, name || file.name.replace(/\.[^/.]+$/, ''), uploadId);
   ...
   */
  private async processAndUpload(file: File, name: string, uploadId: string): Promise<Image> {
  ...
  private async handlePlayerUpload(
    data: ArrayBuffer, 
    metadata: { name: string; width: number; height: number; mimeType: string; fileSize: number },
    uploadId: string,
    peerId: string
  ): Promise<void> {
    try {
      ...
      console.log(`[ImageService] Created image ${image.Name} from player upload`);
    } catch (error) {
