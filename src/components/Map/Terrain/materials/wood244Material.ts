...
function installWood244Shader(
	...
	material.onBeforeCompile = (shader) => {
		...
		shader.vertexShader = shader.vertexShader.replace(
			'#include <common>',
			['#include <common>', ...woodShaderHeader(), ...MOVEMENT_HIGHLIGHT_VERTEX_HEADER, ...HERO_OCCLUSION_VERTEX_HEADER].join('\n')
		);
		shader.vertexShader = shader.vertexShader.replace(
			'#include <begin_vertex>',
			['#include <begin_vertex>', ...woodBeginVertex(), ...MOVEMENT_HIGHLIGHT_VERTEX_BEGIN, ...HERO_OCCLUSION_VERTEX_BEGIN].join('\n')
		);
		shader.fragmentShader = shader.fragmentShader.replace(
			'#include <common>',
			['#include <common>', ...woodFragmentHeader(performanceMode), ...MOVEMENT_HIGHLIGHT_FRAGMENT_HEADER, ...HERO_OCCLUSION_FRAGMENT_HEADER].join('\n')
		);
		shader.fragmentShader = shader.fragmentShader.replace(
			'#include <color_fragment>',
...
export const createWood244Material: MaterialFactory = (
	...
	installWood244Shader(material, texture, voxelAo, movementHighlight, heroOcclusion, performanceMode);
