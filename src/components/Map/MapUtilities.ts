...
export function rotXY(
  ...
  switch (o) {
    ...
    case 1: return { rx: y,           ry: W - 1 - x   };
    case 2: return { rx: W - 1 - x,   ry: L - 1 - y   };
    case 3: return { rx: L - 1 - y,   ry: x           };
  }
}
...
export function screenEastNeighbor(
  ...
  switch (o) {
    case 0: return { nx: x + 1, ny: y     };
    case 1: return { nx: x,     ny: y + 1 };
    case 2: return { nx: x - 1, ny: y     };
    case 3: return { nx: x,     ny: y - 1 };
  }
}
...
function unrotateXY(
  ...
  switch (o) {
    ...
    case 1: return { x: W - 1 - ry, y: rx };
    case 2: return { x: W - 1 - rx, y: L - 1 - ry };
    case 3: return { x: ry, y: L - 1 - rx };
  }
}
 ...
 */
function screenToIso(screenX: number, screenY: number): { isoX: number; isoY: number } {
...
export function screenToTile(
  ...
  for (let dy = -SEARCH_RADIUS; dy <= SEARCH_RADIUS; dy++) {
    for (let dx = -SEARCH_RADIUS; dx <= SEARCH_RADIUS; dx++) {
      ...
      // Bounds check
      if (checkX < 0 || checkX >= width || checkY < 0 || checkY >= length) continue;
