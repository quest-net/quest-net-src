...
export function AttributeEditor({ attributes, onChange, readOnly = false }: AttributeEditorProps) {
  ...
  return (
    <div className="space-y-4">
        ...
        <table className="table table-sm">
          ...
          <tbody>
            ...
            {Object.keys(attributes).length === 0 && (
              <tr>
                <td colSpan={readOnly ? 2 : 3} className="text-center italic text-base-content/60">
      ...
      {!readOnly && (
        <button onClick={handleAdd} disabled={readOnly} className="btn btn-sm btn-outline btn-primary">
          Add Attribute
        </button>
      )}
    </div>
