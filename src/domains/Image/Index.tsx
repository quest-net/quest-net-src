...
export function ImageIndex() {
  ...
  const handleBulkUpdateImageTags = (updates: Array<{ itemId: string; newTags: string[] }>) => {
  ...
  const items: IndexViewItem[] = campaign.Images.map(image => ({
    id: image.Id,
    label: image.Name,
    details: `${formatFileSize(image.FileSize)} • ${image.Width}×${image.Height}`,
    imageId: image.Id,
    tags: image.Tags || []
        ...
        renderEditForm={(item) => {
          ...
          return (
            <div className="space-y-4">
              ...
              <div className="w-full bg-base-200 rounded-lg overflow-hidden flex items-center justify-center" style={{ maxHeight: '70vh' }}>
                <ImageDisplay
                  imageId={image.Id}
