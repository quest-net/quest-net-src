...
function ItemForm({ data, onChange }: ItemFormProps) {
  ...
  return (
    <>
        ...
        <FormGrid cols={2}>
          <FormField label="Max Uses">
            <input
              ...
              onChange={(e) => {
                ...
                handleFieldChange('MaxUses', Number.isFinite(val as number) ? val : undefined);
              }}
              className="input input-bordered w-full"
              min={0}
          ...
          <FormField label="Is Equippable">
            <input
              ...
              onChange={(e) => handleFieldChange('IsEquippable', e.target.checked)}
      ...
      <FormSection
        title="Tags"
        description="Organize this item with tags"
      >
        <TagEditor
          tags={data.Tags || []}
