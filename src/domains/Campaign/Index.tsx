...
export function CampaignIndex() {
  ...
  return (
    <div style={{ padding: '20px', maxWidth: '800px', margin: '0 auto' }}>
      ...
      {showCreateForm && (
        <div style={{ marginBottom: '20px', padding: '15px', border: '2px solid #4CAF50', borderRadius: '5px' }}>
          ...
          <button 
            ...
            style={{ padding: '8px 20px', cursor: 'pointer', background: '#4CAF50', color: 'white', border: 'none' }}
          >
            Create
          </button>
        </div>
      ...
      {showJoinForm && (
        <div style={{ marginBottom: '20px', padding: '15px', border: '2px solid #2196F3', borderRadius: '5px' }}>
          ...
          <button 
            ...
            style={{ padding: '8px 20px', cursor: 'pointer', background: '#2196F3', color: 'white', border: 'none' }}
          >
            Join
          </button>
        </div>
          ...
          <div className="flex-col gap-4">
            {context.Campaigns.map(campaign => (
              <div 
                ...
                <div><strong>Room Code:</strong> {campaign.RoomCode}</div>
                <div><strong>Last Activity:</strong> {campaign.Log.length > 0 ? new Date(campaign.Log[campaign.Log.length - 1].Timestamp).toLocaleString() : 'Never'}</div>
                <div>
                  <button 
                    onClick={(e) => {
                      e.stopPropagation();
