...
export function CampaignView() {
  ...
  useEffect(() => {
    ...
    console.log(`[CampaignView] Initializing in ${isDM ? 'DM' : 'Player'} mode for: ${identifier}`);
    ...
    async function initialize() {
      try {
        ...
        // =====================================================================
        let campaign = CampaignActions.findCampaignByIdentifier(identifier, context);
        ...
        if (isDM) {
          ...
          if (!campaign) {
            console.log('[CampaignView] Player: Campaign not in context, checking localStorage');
            
            // Check if we have a saved version from a previous session
            // Players save received campaigns as: campaign_${roomCode}
            const savedCampaign = localStorage.getItem(`campaign_${identifier}`);
            if (savedCampaign) {
              try {
                campaign = JSON.parse(savedCampaign) as Campaign;
                context.Campaigns.push(campaign);
                triggerContextUpdate();
                console.log('[CampaignView] Player: Loaded campaign from localStorage');
              } catch (error) {
                console.error('[CampaignView] Failed to parse saved campaign:', error);
              }
            }
          }
          ...
          if (campaign) {
            console.log(`[CampaignView] Player found campaign: ${campaign.Name}`);
          } else {
            console.log('[CampaignView] Player waiting for DM to send initial state');
          }
        }
        ...
        if (!isDM && !campaign) {
          ...
          const firstUpdatePromise = new Promise<void>(resolve => {
            service?.onFirstUpdate(() => {
              if (isSubscribed) {
                console.log('[CampaignView] Player received initial state via callback.');
                resolve();
              }
            });
          });
          ...
          Promise.race([firstUpdatePromise, timeoutPromise])
            ...
            .catch((error) => {
              ...
              setState({ 
                status: 'error', 
                errorMessage: 'Could not connect to the DM. Please verify the room code and try again.' 
              });
            });
        } else {
  ...
  if (state.status === 'waiting-for-dm') {
    return (
      <div className="p-8 text-center">
        ...
        <p className="mb-4">Connected to room. Waiting for the DM to start the session.</p>
