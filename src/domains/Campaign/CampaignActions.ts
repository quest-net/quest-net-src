...
function generateRoomCode(): string {
	const adjectives = ['swift', 'brave', 'dark', 'golden', 'silent', 'ancient', 'mystic', 'iron'];
	const nouns = ['dragon', 'sword', 'shield', 'tower', 'forest', 'mountain', 'ocean', 'phoenix'];
...
export const CampaignActions = {
	...
	findCampaignByIdentifier(identifier: string | undefined, context: Context): Campaign | undefined {
		if (!identifier) {
			return undefined;
		}
	 ...
	 */
	create(params: { name: string; roomCode?: string }, context: Context): Campaign {
	...
	delete(params: { campaignId: string }, context: Context): void {
		const index = context.Campaigns.findIndex(c => c.Id === params.campaignId);
	 ...
	 */
	edit(params: { campaignId: string; updates: Partial<Campaign> }, context: Context): void {
	 ...
	 */
	async importFromFile(params: { file: File }, context: Context): Promise<Campaign> {
		try {
			...
			throw new Error(`Failed to import campaign: ${error instanceof Error ? error.message : 'Invalid JSON'}`);
		}
	}
};