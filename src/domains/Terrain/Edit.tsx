...
function TerrainForm({ data, onChange }: TerrainFormProps) {
  ...
  return (
    <>
        ...
        <FormGrid cols={2}>
          ...
          <FormField label="Width" hint="(1-48 tiles)">
            <input
              type="number"
              value={data.Width}
              onChange={(e) => handleSizeChange(Number(e.target.value), data.Length)}
              className="input input-bordered w-full"
              min={1}
              max={48}
            />
          ...
          <FormField label="Length" hint="(1-48 tiles)">
            <input
              type="number"
              value={data.Length}
              onChange={(e) => handleSizeChange(data.Width, Number(e.target.value))}
              className="input input-bordered w-full"
              min={1}
              max={48}
            />
      ...
      <FormSection 
        title="Tags"
        description="Organize terrains with tags"
      >
        <TagEditor
          tags={data.Tags || []}
